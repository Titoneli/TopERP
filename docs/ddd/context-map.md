# TopERP CRM - Context Map (DDD)

| Metadado | Valor |
|----------|-------|
| **Versão** | 2.1 |
| **Data** | 29/01/2026 |
| **Autor** | Product Owner |
| **Status** | Aprovado |
| **Metodologia** | Domain-Driven Design |

---

## 1. Visão Geral

Este documento define o **Context Map** do sistema TopERP CRM, identificando os Bounded Contexts, seus relacionamentos e padrões de integração seguindo os princípios de Domain-Driven Design (DDD).

---

## 2. Diagrama do Context Map

```
┌─────────────────────────────────────────────────────────────────────────────────┐
│                              TOPIERP CRM - CONTEXT MAP                          │
│                                    Versão 2.0                                   │
├─────────────────────────────────────────────────────────────────────────────────┤
│                                                                                 │
│  ┌─────────────────┐                                                            │
│  │   AUTENTICAÇÃO  │◄──────────────────────────────────────────────────────┐    │
│  │    (Generic)    │                                                       │    │
│  │   CRM-AUT       │                                                       │    │
│  └────────┬────────┘                                                       │    │
│           │ JWT Token                                                      │    │
│           ▼                                                                │    │
│  ┌─────────────────┐     U/D      ┌─────────────────┐                      │    │
│  │   CADASTROS     │◄────────────►│     LEADS       │                      │    │
│  │   BÁSICOS       │              │     (Core)      │                      │    │
│  │  (Supporting)   │              │   CRM-LED       │                      │    │
│  │   CRM-CAD       │              └────────┬────────┘                      │    │
│  └─────────────────┘                       │                               │    │
│         │                                  │ Lead ID (SK)                  │    │
│         │ FIPE                             ▼                               │    │
│         │                         ┌─────────────────┐                      │    │
│         │                         │    COTAÇÕES     │                      │    │
│  ┌──────▼──────────┐              │     (Core)      │                      │    │
│  │   API FIPE      │◄─────────────│   CRM-COT       │                      │    │
│  │   (External)    │   ACL        └────────┬────────┘                      │    │
│  └─────────────────┘                       │                               │    │
│                                            │ Cotação ID                    │    │
│                                            ▼                               │    │
│                               ┌─────────────────────┐                      │    │
│                               │   FUNIL DE LEADS    │                      │    │
│                               │      (Core)         │                      │    │
│                               │    CRM-FUN          │                      │    │
│                               └────────┬────────────┘                      │    │
│                                        │                                   │    │
│  ┌─────────────────────────────────────┼───────────────────────────────┐   │    │
│  │          FLUXO SEQUENCIAL DO FUNIL (State Machine)                  │   │    │
│  │                                     │                               │   │    │
│  │    ┌────────────────────────────────▼────────────────────────────┐  │   │    │
│  │    │                                                             │  │   │    │
│  │    │  ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌────────┐│  │   │    │
│  │    │  │PROPOSTAS │───►│PAGAMENTOS│───►│VISTORIAS │───►│ANÁLISE ││  │   │    │
│  │    │  │ CRM-PRO  │    │ CRM-PAG  │    │ CRM-VIS  │    │CRM-ANA ││  │   │    │
│  │    │  │  (Core)  │    │  (Core)  │    │  (Core)  │    │ (Core) ││  │   │    │
│  │    │  └──────────┘    └────┬─────┘    └──────────┘    └───┬────┘│  │   │    │
│  │    │                       │                              │     │  │   │    │
│  │    │                       │                              ▼     │  │   │    │
│  │    │                       │                     ┌─────────────┐│  │   │    │
│  │    │                       │                     │CONCRETIZADA ││  │   │    │
│  │    │                       │                     │  (Outcome)  ││  │   │    │
│  │    │                       │                     └─────────────┘│  │   │    │
│  │    └─────────────────────────────────────────────────────────────┘  │   │    │
│  │                            │                                        │   │    │
│  │                            │ Integração                             │   │    │
│  │                            ▼                                        │   │    │
│  │              ┌─────────────────────────┐                            │   │    │
│  │              │     BANCO DIGITAL       │                            │   │    │
│  │              │      (External)         │                            │   │    │
│  │              │    PIX / Boleto         │                            │   │    │
│  │              └─────────────────────────┘                            │   │    │
│  └─────────────────────────────────────────────────────────────────────┘   │    │
│                                                                            │    │
│  ┌───────────────────────────────────────────────────────────────────────┐ │    │
│  │                      CONTEXTOS DE SUPORTE                             │ │    │
│  │                                                                       │ │    │
│  │  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐             │ │    │
│  │  │   TAREFAS &   │  │COMISSIONAMENTO│  │ APP CONSULTOR │             │ │    │
│  │  │   AGENDAS     │  │ & REMUNERAÇÃO │  │   (Mobile)    │             │ │    │
│  │  │  CRM-TAR      │  │   CRM-COM     │  │   CRM-APP     │             │ │    │
│  │  │ (Supporting)  │  │ (Supporting)  │  │ (Supporting)  │             │ │    │
│  │  └───────────────┘  └───────────────┘  └───────────────┘             │ │    │
│  │                                                                       │ │    │
│  │  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐             │ │    │
│  │  │  FOLLOW-UP    │  │  ASSINATURA   │  │   PÓS-VENDA   │             │ │    │
│  │  │  & CAPTAÇÃO   │  │   DIGITAL     │  │               │             │ │    │
│  │  │   CRM-CAP     │  │   CRM-ASS     │  │   CRM-POS     │             │ │    │
│  │  │ (Supporting)  │  │ (Supporting)  │  │ (Supporting)  │             │ │    │
│  │  └───────────────┘  └───────────────┘  └───────────────┘             │ │    │
│  └───────────────────────────────────────────────────────────────────────┘ │    │
│                                                                            │    │
│  ┌───────────────────────────────────────────────────────────────────────┐ │    │
│  │                    CONTEXTOS DE LEITURA (CQRS)                        │ │    │
│  │                                                                       │ │    │
│  │  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐             │ │    │
│  │  │   DASHBOARD   │  │  RELATÓRIOS   │  │   AUDITORIA   │             │ │    │
│  │  │  (Read-Only)  │  │   & KPIs      │  │   (Logging)   │             │ │    │
│  │  │   CRM-DAS     │  │   CRM-REL     │  │   CRM-AUD     │             │ │    │
│  │  │    CQRS       │  │    CQRS       │  │   (Generic)   │             │ │    │
│  │  └───────────────┘  └───────────────┘  └───────────────┘             │ │    │
│  └───────────────────────────────────────────────────────────────────────┘ │    │
│                                                                            │    │
├────────────────────────────────────────────────────────────────────────────┘    │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐    │
│  │                    SHARED KERNEL - BANCO PostgreSQL                     │    │
│  │                                                                         │    │
│  │   ┌─────────────┐    ┌─────────────┐    ┌─────────────┐                │    │
│  │   │ COR_PESSOA  │    │ COR_CLIENTE │    │ COR_VEICULO │                │    │
│  │   └─────────────┘    └─────────────┘    └─────────────┘                │    │
│  │                                                                         │    │
│  │   Compartilhado entre CRM e TopERP Core                                │    │
│  └─────────────────────────────────────────────────────────────────────────┘    │
│                                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────────┐    │
│  │                    INTEGRAÇÃO LEGACY - SGA                              │    │
│  │                                                                         │    │
│  │   ┌─────────────────────────────────────────────────────────────────┐  │    │
│  │   │                    ANTI-CORRUPTION LAYER                        │  │    │
│  │   │                    (Adapter Pattern)                            │  │    │
│  │   └─────────────────────────────────────────────────────────────────┘  │    │
│  │                              │                                         │    │
│  │                              ▼                                         │    │
│  │   ┌─────────────────────────────────────────────────────────────────┐  │    │
│  │   │                    SISTEMA SGA (Legacy)                         │  │    │
│  │   └─────────────────────────────────────────────────────────────────┘  │    │
│  └─────────────────────────────────────────────────────────────────────────┘    │
│                                                                                 │
└─────────────────────────────────────────────────────────────────────────────────┘
```

---

## 3. Bounded Contexts

### 3.1 Tabela de Bounded Contexts

| Código | Bounded Context | Tipo | Responsabilidade | Status |
|--------|-----------------|------|------------------|--------|
| CRM-AUT | Autenticação | Generic | Login, JWT, permissões | ✅ Documentado |
| CRM-CAD | Cadastros Básicos | Supporting | CRUD Planos, Serviços, Tabelas, FIPE | 📋 Planejado |
| CRM-LED | Leads | Core | Captação e gestão de leads/prospects | ✅ Documentado |
| CRM-COT | Cotações | Core | Simulação e cálculo de preços | ⚠️ Parcial |
| CRM-FUN | Funil de Leads | Core | Gestão de pipeline e conversão | ✅ Documentado |
| CRM-PRO | Propostas | Core | Geração e envio de propostas | 📋 Planejado |
| CRM-PAG | Pagamentos | Core | PIX, boleto, integração bancária | 📋 Planejado |
| CRM-VIS | Vistorias | Core | Agendamento e execução de vistorias | 📋 Planejado |
| CRM-ANA | Análise | Core | Análise documental e aprovação | 📋 Planejado |
| CRM-FIN | Financeiro Consultor | Core | Saques, NF, comissões, motor regras | ✅ Documentado |
| CRM-MTR | Motor de Regras | Shared Kernel | Cálculo de comissões, descontos, scores | ✅ Documentado |
| CRM-TAR | Tarefas & Agendas | Supporting | Gestão de atividades e calendário | 📋 Planejado |
| CRM-COM | Comissionamento | Supporting | Cálculo de comissões e remuneração | 🔄 Absorvido por CRM-FIN |
| CRM-APP | App Consultor | Supporting | Aplicativo mobile para consultores | 📋 Planejado |
| CRM-CAP | Captação & Follow-Up | Supporting | Multicanal e acompanhamento | 📋 Planejado |
| CRM-ASS | Assinatura Digital | Supporting | Assinatura eletrônica de contratos | 📋 Planejado |
| CRM-POS | Pós-Venda | Supporting | Acompanhamento pós-conversão | 📋 Planejado |
| CRM-DAS | Dashboard | CQRS Read | Visualização consolidada de dados | 📋 Planejado |
| CRM-REL | Relatórios | CQRS Read | Geração de relatórios e KPIs | 📋 Planejado |
| CRM-AUD | Auditoria | Generic | Log de ações e rastreabilidade | 📋 Planejado |

### 3.2 Legenda de Status

- ✅ **Documentado**: User Stories e especificações completas
- ⚠️ **Parcial**: Documentação iniciada, em desenvolvimento
- 📋 **Planejado**: Identificado no roadmap, aguardando documentação

---

## 4. Relacionamentos entre Contextos

### 4.1 Padrões de Integração

| Contexto Upstream | Contexto Downstream | Padrão | Descrição |
|-------------------|---------------------|--------|-----------|
| CRM-AUT | Todos | Published Language | JWT Token compartilhado |
| CRM-LED | CRM-COT | Customer/Supplier | Lead ID como referência |
| CRM-COT | CRM-FUN | Customer/Supplier | Cotação vinculada ao funil |
| CRM-FUN | CRM-PRO | State Machine | Transição de estado |
| CRM-PRO | CRM-PAG | State Machine | Proposta aceita → Pagamento |
| CRM-PAG | CRM-VIS | State Machine | Pagamento confirmado → Vistoria |
| CRM-VIS | CRM-ANA | State Machine | Vistoria realizada → Análise |
| CRM-ANA | CRM-FIN | Event | Venda concretizada → Crédito comissão |
| CRM-MTR | CRM-FIN | Shared Kernel | Motor de Regras para cálculos de comissão |
| CRM-MTR | CRM-COT | Shared Kernel | Motor de Regras para descontos e preços |
| CRM-MTR | CRM-LED | Shared Kernel | Motor de Regras para score de leads |
| CRM-MTR | CRM-POS | Shared Kernel | Motor de Regras para bonificações |
| CRM-FIN | CRM-PAG | Event | Estorno → Devolução cliente |
| CRM-FIN | SEFAZ | ACL | Emissão NF-e/NFS-e |
| CRM-FIN | MFG/Sankhya | ACL | Ordens de pagamento |
| CRM-FIN | Banco Digital | ACL | PIX, saques |
| CRM-FIN | Sistema Contábil | ACL | Lançamentos contábeis |
| CRM-CAD | CRM-COT | Upstream/Downstream | Dados de preços e planos |
| CRM-* | CRM-DAS | CQRS | Read Model agregado |
| CRM-* | CRM-REL | CQRS | Read Model para relatórios |
| CRM-* | CRM-AUD | Event Sourcing | Eventos de domínio |
| API FIPE | CRM-CAD | ACL | Anti-Corruption Layer |
| Banco Digital | CRM-PAG | ACL | Anti-Corruption Layer |
| SGA | CRM-* | ACL | Anti-Corruption Layer |

### 4.2 Shared Kernel

O CRM e o TopERP Core compartilham o mesmo banco de dados PostgreSQL com as seguintes tabelas principais:

```
┌─────────────────────────────────────────────────────────────────┐
│                     SHARED KERNEL                               │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────────┐     ┌─────────────┐     ┌─────────────┐       │
│  │ COR_PESSOA  │────►│ COR_CLIENTE │────►│ COR_VEICULO │       │
│  │             │     │             │     │             │       │
│  │ - ID        │     │ - ID        │     │ - ID        │       │
│  │ - Nome      │     │ - Pessoa_ID │     │ - Cliente_ID│       │
│  │ - CPF/CNPJ  │     │ - Tipo      │     │ - Placa     │       │
│  │ - Contato   │     │ - Status    │     │ - FIPE      │       │
│  └─────────────┘     └─────────────┘     └─────────────┘       │
│                                                                 │
│  Regras:                                                        │
│  • Lead = Prospect (mesmo conceito, ID compartilhado)           │
│  • Entidade única COR_PESSOA para ambos os sistemas            │
│  • Sincronização em tempo real via triggers/eventos            │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 5. Fluxo do Funil de Vendas

### 5.1 State Machine do Funil

```
┌──────────────────────────────────────────────────────────────────────────────┐
│                        FUNIL DE VENDAS - STATE MACHINE                       │
├──────────────────────────────────────────────────────────────────────────────┤
│                                                                              │
│  [LEAD CAPTADO]                                                              │
│        │                                                                     │
│        ▼                                                                     │
│  ┌──────────┐   criar    ┌──────────┐   aceitar   ┌──────────┐              │
│  │  LEADS   │───────────►│ COTAÇÃO  │────────────►│ PROPOSTA │              │
│  │ CRM-LED  │            │ CRM-COT  │             │ CRM-PRO  │              │
│  └──────────┘            └──────────┘             └────┬─────┘              │
│                                recusar                 │                     │
│                                   │                    │ enviar              │
│                                   ▼                    ▼                     │
│                          ┌──────────────┐      ┌──────────┐                 │
│                          │    PERDIDO   │      │PAGAMENTO │                 │
│                          │   (Outcome)  │      │ CRM-PAG  │                 │
│                          └──────────────┘      └────┬─────┘                 │
│                                                     │                        │
│                                                     │ confirmar              │
│                                                     ▼                        │
│                                               ┌──────────┐                   │
│                                               │ VISTORIA │                   │
│                                               │ CRM-VIS  │                   │
│                                               └────┬─────┘                   │
│                                                    │                         │
│                                                    │ realizar                │
│                                                    ▼                         │
│                                               ┌──────────┐                   │
│                                               │ ANÁLISE  │                   │
│                                               │ CRM-ANA  │                   │
│                                               └────┬─────┘                   │
│                                                    │                         │
│                              ┌─────────────────────┼─────────────────────┐   │
│                              │                     │                     │   │
│                              ▼                     ▼                     ▼   │
│                       ┌──────────┐          ┌──────────┐          ┌────────┐│
│                       │ APROVADO │          │ PENDÊNCIA│          │REPROVADO│
│                       │(Concretiz)│          │(Correção)│          │(Perdido)│
│                       └──────────┘          └──────────┘          └────────┘│
│                                                                              │
└──────────────────────────────────────────────────────────────────────────────┘
```

### 5.2 Eventos de Domínio do Funil

| Evento | Contexto Origem | Contextos Interessados |
|--------|-----------------|------------------------|
| `LeadCaptado` | CRM-LED | CRM-DAS, CRM-AUD, CRM-TAR |
| `CotacaoCriada` | CRM-COT | CRM-LED, CRM-DAS, CRM-AUD |
| `PropostaEnviada` | CRM-PRO | CRM-LED, CRM-PAG, CRM-AUD |
| `PagamentoConfirmado` | CRM-PAG | CRM-VIS, CRM-COM, CRM-AUD |
| `VistoriaRealizada` | CRM-VIS | CRM-ANA, CRM-DAS, CRM-AUD |
| `AnaliseAprovada` | CRM-ANA | CRM-POS, CRM-COM, CRM-AUD |
| `NegociacaoConcretizada` | CRM-ANA | CRM-POS, CRM-DAS, CRM-REL |
| `NegociacaoPerdida` | CRM-* | CRM-DAS, CRM-REL, CRM-AUD |

---

## 6. Canais de Captação (Multicanal)

```
┌─────────────────────────────────────────────────────────────────┐
│                    CAPTAÇÃO MULTICANAL                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐               │
│  │WhatsApp │ │  Site   │ │Facebook │ │Instagram│               │
│  └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘               │
│       │           │           │           │                     │
│       └───────────┴─────┬─────┴───────────┘                     │
│                         │                                       │
│                         ▼                                       │
│  ┌─────────┐ ┌─────────────────────────────┐ ┌─────────┐       │
│  │Indicação│─┤      API DE CAPTAÇÃO        ├─│  Evento │       │
│  └─────────┘ │       (CRM-CAP)             │ └─────────┘       │
│              └──────────────┬──────────────┘                    │
│  ┌─────────┐                │                ┌─────────┐       │
│  │ Landing │────────────────┴────────────────│Telefone │       │
│  │  Page   │                                 │         │       │
│  └─────────┘                                 └─────────┘       │
│                             │                                   │
│                             ▼                                   │
│                   ┌─────────────────┐                           │
│                   │   CRM-LEADS     │                           │
│                   │   (Unificado)   │                           │
│                   └─────────────────┘                           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## 7. Integrações Externas

### 7.1 Anti-Corruption Layers

| Sistema Externo | ACL | Responsabilidade |
|-----------------|-----|------------------|
| **API FIPE** | FipeAdapter | Consulta de valores de veículos |
| **Banco Digital** | BancoDigitalAdapter | PIX, boleto, confirmação |
| **SGA (Legacy)** | SGAAdapter | Sincronização de dados legados |
| **WhatsApp API** | WhatsAppAdapter | Envio de mensagens e captação |
| **Email Service** | EmailAdapter | Notificações e envio de propostas |
| **VistorIA** | VistoriaAdapter | Integração com App VistorIA |
| **N8N (Automação)** | AutomacaoAdapter | Workflows e integrações automatizadas |

### 7.2 Diagrama de Integração

```
┌───────────────────────────────────────────────────────────────┐
│                    INTEGRAÇÕES EXTERNAS                       │
├───────────────────────────────────────────────────────────────┤
│                                                               │
│  ┌─────────────────┐         ┌─────────────────┐             │
│  │   API FIPE      │◄───ACL──│   CRM-CAD       │             │
│  │   (External)    │         │   Cadastros     │             │
│  └─────────────────┘         └─────────────────┘             │
│                                                               │
│  ┌─────────────────┐         ┌─────────────────┐             │
│  │  Banco Digital  │◄───ACL──│   CRM-PAG       │             │
│  │  PIX/Boleto     │         │   Pagamentos    │             │
│  └─────────────────┘         └─────────────────┘             │
│                                                               │
│  ┌─────────────────┐         ┌─────────────────┐             │
│  │  WhatsApp API   │◄───ACL──│   CRM-CAP       │             │
│  │                 │         │   Captação      │             │
│  └─────────────────┘         └─────────────────┘             │
│                                                               │
│  ┌─────────────────┐         ┌─────────────────┐             │
│  │  Sistema SGA    │◄───ACL──│   CRM-*         │             │
│  │  (Legacy)       │         │   (Todos)       │             │
│  └─────────────────┘         └─────────────────┘             │
│                                                               │
└───────────────────────────────────────────────────────────────┘
```

---

## 8. Padrões Arquiteturais

### 8.1 CQRS (Command Query Responsibility Segregation)

Os contextos **CRM-DAS** (Dashboard) e **CRM-REL** (Relatórios) são implementados como **Read Models** usando o padrão CQRS:

```
┌─────────────────────────────────────────────────────────────┐
│                        CQRS PATTERN                         │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  ┌─────────────┐          ┌─────────────┐                  │
│  │  COMMANDS   │          │   QUERIES   │                  │
│  │  (Write)    │          │   (Read)    │                  │
│  └──────┬──────┘          └──────┬──────┘                  │
│         │                        │                          │
│         ▼                        ▼                          │
│  ┌─────────────┐          ┌─────────────┐                  │
│  │   CRM-*     │          │  CRM-DAS    │                  │
│  │ (Domínios)  │──Events─►│  CRM-REL    │                  │
│  │             │          │ (Read-Only) │                  │
│  └──────┬──────┘          └──────┬──────┘                  │
│         │                        │                          │
│         ▼                        ▼                          │
│  ┌─────────────┐          ┌─────────────┐                  │
│  │Write Database│          │Read Database │                  │
│  │ (PostgreSQL)│          │(Views/Cache)│                  │
│  └─────────────┘          └─────────────┘                  │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

### 8.2 Event Sourcing (Auditoria)

O contexto **CRM-AUD** utiliza Event Sourcing para manter histórico completo de ações:

- Todos os eventos de domínio são persistidos
- Permite reconstrução de estado em qualquer ponto no tempo
- Garante rastreabilidade para compliance

---

## 9. Roadmap de Documentação

### 9.1 Priorização por Versão

| Versão | Contextos | Data Alvo |
|--------|-----------|-----------|
| v1.0 | CRM-AUT, CRM-LED, CRM-COT, CRM-FUN | Q1/2026 |
| v2.0 | CRM-PRO, CRM-PAG, CRM-VIS, CRM-ANA | Q2/2026 |
| v3.0 | CRM-TAR, CRM-CAP, CRM-ASS, CRM-COM | Q3/2026 |
| v4.0 | CRM-APP, CRM-DAS, CRM-REL, CRM-POS | Q4/2026 |

### 9.2 Próximos Passos

1. ✅ Context Map definido e aprovado
2. 📋 Detalhar Bounded Context CRM-PAG (Pagamentos)
3. 📋 Detalhar Bounded Context CRM-VIS (Vistorias)
4. 📋 Detalhar Bounded Context CRM-ANA (Análise)
5. 📋 Detalhar Bounded Context CRM-PRO (Propostas)
6. 📋 Atualizar READMEs existentes com padrão DDD
7. 📋 Criar User Stories para contextos faltantes

---

## 10. Glossário DDD

| Termo | Definição |
|-------|-----------|
| **Bounded Context** | Fronteira explícita onde um modelo de domínio é definido e aplicável |
| **Aggregate** | Cluster de objetos de domínio tratados como unidade |
| **Entity** | Objeto com identidade única e ciclo de vida |
| **Value Object** | Objeto imutável sem identidade própria |
| **Domain Event** | Registro de algo que aconteceu no domínio |
| **Repository** | Abstração para persistência de agregados |
| **Shared Kernel** | Código/modelo compartilhado entre contextos |
| **ACL** | Anti-Corruption Layer - proteção contra modelos externos |
| **CQRS** | Separação entre comandos (escrita) e consultas (leitura) |
| **Upstream/Downstream** | Direção de dependência entre contextos |

---

## 11. Histórico de Alterações

| Data | Versão | Autor | Alteração |
|------|--------|-------|-----------|
| 29/01/2026 | 2.1 | Product Owner | **CRM-MTR (Motor de Regras)**: Novo Bounded Context tipo Shared Kernel. Componente genérico reutilizável por FIN, COT, LED, POS. 6 User Stories, 178 SP |
| 25/01/2026 | 2.0 | Product Owner | Adicionados ACLs VistorAI e N8N (origem: visao-produto-crm.md) |
| 22/01/2026 | 1.0 | Product Owner | Versão inicial aprovada com 18 Bounded Contexts |

---

## 12. Referências

- [Product Backlog](../backlog/product-backlog.md)
- [Roadmap de Releases](../releases/roadmap.md)
- [Glossário do Sistema](../glossario/glossario.md)
- [Visão do Produto CRM](../visao-produto-crm.md)
